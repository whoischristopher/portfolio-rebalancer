...
class User(UserMixin, db.Model):
    ...
    preferences = db.relationship('AssetClassPreference', backref='user', lazy=True, cascade='all, delete-orphan')
...
class Holding(db.Model):
    ...
    asset_class = db.Column(db.String(100), index=True)  # 'Canadian Equity', 'US Equity', 'Bonds', etc.
...
class Target(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, index=True)
    asset_class = db.Column(db.String(100), nullable=False)
